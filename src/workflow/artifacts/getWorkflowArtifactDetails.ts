...
type ListWorkflowRunArtifacts = RestEndpointMethodTypes['actions']['listWorkflowRunArtifacts']
...
> {
  return useOctokit(async octokit => {
    ...
    const artifacts: ListWorkflowRunArtifacts['response']['data']['artifacts'] = await octokit.paginate(
      artifactsUrl,
      {
        per_page: 100
      }
